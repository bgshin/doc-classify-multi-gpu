...
class W2V_LEX_CNN_CONCAT(object):
    """
    A CNN for text classification.
    Uses an embedding layer, followed by a convolutional, max-pooling and softmax layer.
    """
    def __init__(
        ...
        self.input_x_concat = tf.concat(2, [self.input_x, self.input_x_lexicon])
        print 'self.input_x_concat', self.input_x_concat
        ...
        # Embedding layer
        with tf.device('/cpu:0'), tf.name_scope("embedding"):
            self.embedded_chars_expanded = tf.expand_dims(self.input_x_concat, -1)
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.name_scope("conv-maxpool-%s" % filter_size):
                # Convolution Layer
                filter_shape = [filter_size, embedding_size+embedding_size_lex, 1, num_filters]
                W = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.1), name="W")
                b = tf.Variable(tf.constant(0.1, shape=[num_filters]), name="b")
...
class W2V_LEX_CNN_CONCAT_A2V(object):
    """
    A CNN for text classification.
    Uses an embedding layer, followed by a convolutional, max-pooling and softmax layer.
    """
    def __init__(
            ...
            attention_depth_w2v, attention_depth_lex, l2_reg_lambda=0.0,
            l1_reg_lambda=0.0):
