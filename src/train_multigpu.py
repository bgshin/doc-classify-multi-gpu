...
def run_train(w2vsource, w2vdim, w2vnumfilters, randomseed, num_epochs, l2_reg_lambda, l1_reg_lambda):
    ...
    with Timer("w2v"):
        w2vmodel = load_w2v(w2vdim, simple_run=False, source=w2vsource)
        # w2vmodel = load_w2v_new(w2vdim)

    x_train, y_train = cnn_data_helpers.load_data_new('trn', w2vmodel, max_len)
    x_dev, y_dev = cnn_data_helpers.load_data_new('dev', w2vmodel, max_len)
    x_test, y_test = cnn_data_helpers.load_data_new('tst', w2vmodel, max_len)
